...
void AttitudeEstimation::updateIMU(){
	...
	if(rotationAxsis.getNorm() && accelNorm < 0.2){
		float cos_phi = accelValue[2] / (1.0);
	...
	Quaternion deltaAccelQuaternion = attitude.invers()*accelQuaternion;
	...
	if(accelNorm < 0.2){
		...
		attitude *= (deltaGyro*(1-coefficient) + deltaAccelQuaternion*(coefficient)).normalize();

		auto res = (deltaAccelQuaternion).normalize();
	}else{
		attitude *= (deltaGyro).normalize();
	}
